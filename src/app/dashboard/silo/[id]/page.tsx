      ...
      <div className="page-header">
        <div>
          ...
          <div className="page-sub" style={{ marginLeft: 22 }}>
            {silo.material} · Serial {sensor?.serial ?? '—'} ·{' '}
            {hoursAgo !== null ? `${hoursAgo}h ago` : 'No readings yet'}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ textAlign: 'center' }}>
              {reading ? (
                <>
                  ...
                  >
                    {[
                      ...
                      {
                        val: reading.cubic_meters
                          ? `${reading.cubic_meters.toFixed(1)} m³`
                          : '—',
                        lbl: 'volume',
                      },
                      {
                        val: `${silo.capacity_kg.toLocaleString()} kg`,
                        lbl: 'capacity',
                      },
                      {
                        val: reading.distance_cm
                ...
                <div style={{ padding: '24px 0', color: '#aab8c0', fontSize: 13 }}>
                  ...
                  <br />
                  Waiting for first sensor transmission.
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ background: '#1a2530', borderRadius: 10, padding: '18px 20px' }}>
            ...
            {[
              ...
              {
                k: 'Suggested order',
                v: `${Math.round((silo.capacity_kg * 0.85) / 1000) * 1000} kg`,
                c: '#4CAF7D',
              },
              ...
              >
                ...
                <span
                  style={{
                    fontSize: 12,
                    color: row.c || 'rgba(255,255,255,0.85)',
                    fontWeight: 500,
                  }}
                >
                  {row.v}
            ...
            {sensor ? (
              <>
                {[
                  {
                    k: 'Connection',
                    v: sensor.status.charAt(0).toUpperCase() + sensor.status.slice(1),
                    color: sensor.status === 'online' ? '#27500A' : '#633806',
                  },
                  {
                    k: 'Battery',
                    v: `${sensor.battery_pct}%`,
                    color:
                      sensor.battery_pct >= 70
                        ? '#27500A'
                  ...
                  {
                    k: 'Signal',
                    v: ['—', 'Weak', 'Fair', 'Good'][sensor.signal_strength] || '—',
                  },
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: 12,
                        fontWeight: 500,
                        color: row.color || '#1a2530',
                        fontFamily: row.k === 'Serial' ? 'monospace' : 'inherit',
                      }}
                    >
                      {row.v}
                    </span>
                ...
                {silo.lat && silo.lng && (
                  <a
                    href={`https://www.google.com/maps?q=${silo.lat},${silo.lng}`}
                    target="_blank"
                    rel="noreferrer"
                    style={{
                      display: 'block',
              ...
              <div style={{ color: '#8a9aaa', fontSize: 13 }}>
                No sensor registered for this silo.
              </div>
            )}
          </div>
        </div>
