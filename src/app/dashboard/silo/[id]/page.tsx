...
export default function SiloPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const silo = SILOS.find(s => s.id === parseInt(id)) || SILOS[0]
  const color = levelColor(silo.pct)
  const dColor = silo.days <= 7 ? '#A32D2D' : silo.days <= 14 ? '#633806' : '#27500A'
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    const base = i === 0 ? silo.pct : i <= 5 ? silo.pct + i * 3 : Math.min(100, silo.pct + i * 2.5 + Math.random() * 8)
    ...
    <>
      ...
      <div className="page-header">
        <div>
          ...
          <div className="page-sub" style={{ marginLeft: 22 }}>{silo.material} · Sensor #{SILOS.indexOf(silo) + 1} · Last reading {silo.reading}</div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="card" style={{ marginBottom: 0 }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 48, fontWeight: 500, color, letterSpacing: -2, margin: '12px 0 4px' }}>{silo.pct}%</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card" style={{ marginBottom: 0 }}>
            <div className="card-header">
              ...
              <Link href={`https://www.google.com/maps?q=${silo.lat},${silo.lng}`} target="_blank" style={{ fontSize: 11, color: '#4A90C4' }}>View on map →</Link>
            </div>
            <div style={{ height: 220, position: 'relative' }}>
              <Line
                data={{ labels, datasets: [
                  { label: 'Level %', data, borderColor: '#4CAF7D', backgroundColor: 'rgba(76,175,125,0.08)', borderWidth: 1.5, pointRadius: 0, fill: true, tension: 0.3 },
                  { label: 'Critical threshold', data: Array(30).fill(20), borderColor: '#E24B4A', borderWidth: 1, borderDash: [4,4], pointRadius: 0, fill: false } as any,
                ]}}
                options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false }, ticks: { font: { size: 9 }, color: '#aab8c0', maxTicksLimit: 8, maxRotation: 0 }, border: { display: false } }, y: { min: 0, max: 100, grid: { color: 'rgba(0,0,0,0.04)' }, ticks: { font: { size: 10 }, color: '#aab8c0', callback: (v) => v + '%' }, border: { display: false } } } }}
              />
            </div>
          </div>

          <div className="card" style={{ marginBottom: 0 }}>
            <div className="card-header"><div className="card-title">Configured alarms</div></div>
            {[
              ...
              <div key={alarm.title} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: '#f7f9f8', borderRadius: 8, marginBottom: 8 }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: alarm.color === 'red' ? '#FCEBEB' : alarm.color === 'amber' ? '#FAEEDA' : '#f7f9f8', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={alarm.color === 'red' ? '#A32D2D' : alarm.color === 'amber' ? '#633806' : '#aab8c0'} strokeWidth="1.5" strokeLinecap="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.73 21a2 2 0 0 1-3.46 0" /></svg>
                </div>
                <div style={{ flex: 1 }}>
                ...
                <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 10, background: alarm.status === 'Active' ? '#FCEBEB' : '#f7f9f8', color: alarm.status === 'Active' ? '#A32D2D' : '#aab8c0', border: alarm.status === 'Inactive' ? '0.5px solid #e8ede9' : 'none' }}>{alarm.status}</span>
              </div>
            ))}
          </div>
        </div>
