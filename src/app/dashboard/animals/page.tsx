...
function getRations(name: string) {
  return RATIONS[name] || [{ material: 'Lactation diet', kgPerHead: 3.0, color: '#4CAF7D' }]
}
...
export default function AnimalsPage() {
  ...
  function dailyCost(g: AnimalGroup): number {
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Herd groups · Feed rations · Cost per head</div>
        </div>
      ...
      <div className="summary-row">
        <div className="sum-card">
          ...
          <div className="sum-sub">{groups.length} groups</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Feed / day</div>
          <div className="sum-val">{(totalFeed / 1000).toFixed(1)} t</div>
          <div className="sum-sub">{Math.round(totalFeed).toLocaleString()} kg total</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Cost / day</div>
          <div className="sum-val">${Math.round(totalCost).toLocaleString()}</div>
          <div className="sum-sub">All groups combined</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Avg $/head/day</div>
          <div className="sum-val green">${totalAnimals > 0 ? (totalCost / totalAnimals).toFixed(2) : '0.00'}</div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: `${on ? '1.5px' : '0.5px'} solid ${on ? '#4CAF7D' : '#e8ede9'}`,
                    borderRadius: 12,
                    padding: '18px 18px',
                    cursor: 'pointer',
                    textAlign: 'left',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {(PHASES[selected.name] || []).map((p, i) => {
                      ...
                      return (
                        <button
                          key={p}
                          onClick={() => setActivePhase(prev => ({ ...prev, [selected.id]: i }))}
                          style={{
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {getRations(selected.name).map(r => {
                      ...
                      return (
                        <tr key={r.material}>
                          ...
                          <td style={{ padding: '12px', borderBottom: '0.5px solid #f0f4f0', fontSize: 14, fontWeight: 600, color: '#1a2530' }}>
                            {r.kgPerHead}
                          </td>
                          <td style={{ padding: '12px', borderBottom: '0.5px solid #f0f4f0' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <td style={{ padding: '12px', borderBottom: '0.5px solid #f0f4f0', fontSize: 12, color: '#8a9aaa' }}>
                            {Math.round(totalKg).toLocaleString()} kg
                          </td>
                          <td style={{ padding: '12px', borderBottom: '0.5px solid #f0f4f0', fontSize: 13, fontWeight: 600, color: '#27500A' }}>
                    ...
                    <tr style={{ background: '#f7f9f8' }}>
                      ...
                      <td style={{ padding: '12px', fontSize: 14, fontWeight: 700, color: '#1a2530' }}>
                        {getRations(selected.name).reduce((s, r) => s + r.kgPerHead, 0).toFixed(1)} kg
                      </td>
                      ...
                      <td style={{ padding: '12px', fontSize: 13, fontWeight: 700, color: '#27500A' }}>
              ...
              <div className="card" style={{ marginBottom: 0 }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', fontSize: 11, color: '#8a9aaa', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.4px', fontWeight: 600 }}>
                    Animal count
                  </label>
                  <input
                    type="number"
                  ...
                  />
                  <div style={{ fontSize: 11, color: '#aab8c0', marginTop: 4 }}>
                    Edit and click away — saves to Supabase
                  </div>
                </div>
