    ...
    <>
      ...
      {groups.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 60, color: '#8a9aaa' }}>
          ...
          <div style={{ fontSize: 13 }}>Animal groups will appear here once added to this farm.</div>
        </div>
      ) : (
        <>
            ...
            {groups.map(g => {
              ...
              return (
                <button key={g.id} onClick={() => setSelectedId(g.id)}
                  style={{ background: on ? '#f4fbf7' : '#fff', border: `${on ? '1.5px' : '0.5px'} solid ${on ? '#4CAF7D' : '#e8ede9'}`, borderRadius: 12, padding: '18px', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}>
