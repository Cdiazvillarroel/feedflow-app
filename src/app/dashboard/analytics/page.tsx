...
export default function AnalyticsPage() {
  ...
  // ── REAL DAILY CONSUMPTION CHART ──────────────────────────────────────────
  // Group readings by silo, compute daily consumption from kg_remaining drops
  ...
  // For each silo, compute day-by-day consumption and accumulate
  Object.values(siloReadingsMap).forEach(siloReadings => {
    const sorted = [...siloReadings].sort(
      (a, b) => new Date(a.recorded_at).getTime() - new Date(b.recorded_at).getTime()
    )
    for (let i = 1; i < sorted.length; i++) {
      const prev = sorted[i - 1]
      const curr = sorted[i]
      const consumed = prev.kg_remaining - curr.kg_remaining
      if (consumed > 0) {
        const day = new Date(curr.recorded_at).toLocaleDateString('en-AU', { day: 'numeric', month: 'short' })
  ...
  silos.forEach(s => {
    ...
    matMap[mat] = (matMap[mat] || 0) + s.kg_remaining
  })
  ...
  const avgConsPerDay  = allDays.length > 0
    ? Math.round(dailyConsumptionData.reduce((a, b) => a + b, 0) / dailyConsumptionData.length)
    : 0

  // ── RESTOCK PREDICTIONS using real days_remaining ─────────────────────────
  const predictions = [...silos]
    .filter(s => s.days_remaining <= 30)
    ...
    .map(s => ({
      ...
      kg:       s.kg_remaining,
      level:    s.level_pct,
      order:    Math.round(s.capacity_kg * 0.85 / 1000) * 1000,
    }))

  const urgStyle = (d: number) =>
    d <= 7  ? { bg: '#FCEBEB', color: '#A32D2D', label: 'Critical' } :
    d <= 14 ? { bg: '#FAEEDA', color: '#633806', label: 'High' }
    ...
    <>
      ...
      <div className="summary-row">
        <div className="sum-card">
          ...
          <div className="sum-val">{(totalKg / 1000).toFixed(1)} t</div>
          <div className="sum-sub">Across all silos</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Avg daily consumption</div>
          <div className="sum-val">{avgConsPerDay > 0 ? `${(avgConsPerDay / 1000).toFixed(1)} t` : `${(totalDailyCons / 1000).toFixed(1)} t`}</div>
          <div className="sum-sub">Real sensor data</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Daily feed cost</div>
          <div className="sum-val">${Math.round(totalDailyCost).toLocaleString()}</div>
          <div className="sum-sub">Based on current prices</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Silos monitored</div>
          <div className="sum-val green">{silos.length}</div>
          <div className="sum-sub">{silos.filter(s => s.alert_level === 'critical').length} critical · {silos.filter(s => s.alert_level === 'low').length} low</div>
        </div>
      </div>
      ...
      <div className="grid-2">
        <div className="card" style={{ marginBottom: 0 }}>
          <div className="card-header">
            <div className="card-title">Daily consumption — real sensor data</div>
            <span style={{ fontSize: 11, color: '#aab8c0' }}>Last {period} days</span>
          </div>
          {allDays.length > 1 ? (
            <div style={{ height: 220, position: 'relative' }}>
              <Line
                ...
                options={{
                  ...
                  scales: {
                    x: { grid: { display: false }, ticks: { font: { size: 9 }, color: '#aab8c0', maxTicksLimit: 10, maxRotation: 0 }, border: { display: false } },
                    y: { grid: { color: 'rgba(0,0,0,0.04)' }, ticks: { font: { size: 10 }, color: '#aab8c0', callback: (v: any) => v >= 1000 ? (v/1000).toFixed(1)+'t' : v }, border: { display: false } },
                  },
                }}
              />
            </div>
            ...
            <div style={{ height: 140, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#aab8c0', fontSize: 13 }}>
              Accumulating readings to build the consumption chart.
            </div>
          )}
        </div>
            ...
            <div style={{ height: 220, position: 'relative' }}>
              <Doughnut
                data={{
                  labels: materials,
                  datasets: [{
                    data: materials.map(m => Math.round(matMap[m] / 1000 * 10) / 10),
                ...
                options={{
                  ...
                  plugins: {
                    ...
                    tooltip: { callbacks: { label: (ctx) => ` ${ctx.label}: ${ctx.parsed} t` } },
                  },
                }}
              />
            </div>
      ...
      <div className="card">
        <div className="card-header">
          <div className="card-title">Silo levels — ranked by availability</div>
          <span style={{ fontSize: 11, color: '#aab8c0' }}>Sorted by kg remaining</span>
        </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#aab8c0' }}>
            ...
            Based on real consumption rates
          </div>
        </div>
        {predictions.length === 0 ? (
          <div style={{ color: '#8a9aaa', fontSize: 13, padding: '16px 0', textAlign: 'center' }}>
            All silos have more than 30 days of feed remaining. ✓
          </div>
        ) : (
            ...
            {predictions.map(p => {
              ...
              return (
                <div key={p.silo} style={{ borderRadius: 12, padding: 16, background: u.bg, border: `0.5px solid ${u.color}33`, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {[
                    { k: 'Level',        v: `${p.level.toFixed(1)}%` },
                    { k: 'Available',    v: `${Math.round(p.kg).toLocaleString()} kg` },
                    { k: 'Order needed', v: `${p.order.toLocaleString()} kg`, bold: true },
                  ].map(r => (
