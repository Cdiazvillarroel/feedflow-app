...
function projPct(silo: SiloWithReading, kgDay: number, days: number) {
  ...
  return Math.max(0, Math.round(remaining / silo.capacity_kg * 100))
}
...
// Stable daily kg/day per silo derived from days_remaining (no random)
function siloKgDay(s: SiloWithReading): number {
  if (s.days_remaining > 0) return Math.round(s.kg_remaining / s.days_remaining)
  return 400
}
...
export default function ForecastPage() {
  ...
  const totalCostDay = silos.reduce((s, silo) => {
    ...
    return s + (siloKgDay(silo) / 1000 * price)
  }, 0)
  ...
  // Stable forecast labels & data (memoized — no random on render)
  const { labels, consData, costData } = useMemo(() => {
    ...
    for (let i = 1; i <= horizon; i++) {
      ...
      // Slightly declining consumption as silos empty
      const decayFactor = 1 - (i / (horizon * 20))
      consData.push(Math.round(totalKgDay * decayFactor))
      costData.push(Math.round(totalCostDay * decayFactor))
    }
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Consumption & cost projections · Based on real sensor data</div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 0, background: '#f0f4f0', borderRadius: 8, padding: 3, marginBottom: 20, width: 'fit-content' }}>
        {[7, 15, 30].map(h => (
          <button key={h} onClick={() => setHorizon(h)} style={{
            padding: '8px 28px', borderRadius: 6, fontSize: 13, fontWeight: 500,
            cursor: 'pointer', border: 'none', fontFamily: 'inherit',
            background: horizon === h ? '#1a2530' : 'transparent',
            color: horizon === h ? '#fff' : '#6a7a8a',
      ...
      <div className="summary-row">
        <div className="sum-card">
          <div className="sum-label">Feed consumed</div>
          <div className="sum-val">{(totalKgDay * horizon / 1000).toFixed(1)} t</div>
          <div className="sum-sub">over {horizon} days</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Projected cost</div>
          <div className="sum-val">${Math.round(totalCostDay * horizon).toLocaleString()}</div>
        ...
        <div className="sum-card">
          <div className="sum-label">Farm level at day {horizon}</div>
          <div className="sum-val">{totalCapKg > 0 ? Math.round(projKg / totalCapKg * 100) : 0}%</div>
          <div className="sum-sub">vs {avgLevelToday}% today</div>
        </div>
      </div>
      ...
      <div style={{ background: '#1a2530', borderRadius: 10, padding: '18px 22px', marginBottom: 16, display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 16, alignItems: 'center' }}>
        ...
        <div>
          <div style={{ fontSize: 14, fontWeight: 500, color: '#fff', marginBottom: 4 }}>
            {silos.length === 0
              ? 'No sensor data yet — connect DigitPlan API to generate real forecasts'
              : urgentSilos > 0
          ...
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>
            Daily consumption: {Math.round(totalKgDay).toLocaleString()} kg/day ·
            Projected spend: ${Math.round(totalCostDay * horizon).toLocaleString()} ·
            Avg days remaining: {avgDays}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 24 }}>
          {[
            { val: urgentSilos,                              lbl: 'urgent',   c: urgentSilos > 0 ? '#F09595' : '#4CAF7D' },
            { val: `$${Math.round(totalCostDay * horizon / 1000)}k`, lbl: 'projected', c: '#FAC775' },
      ...
      <div className="grid-2">
        <div className="card" style={{ marginBottom: 0 }}>
          <div className="card-header">
            <div className="card-title">Consumption forecast</div>
            <span style={{ fontSize: 11, color: '#aab8c0' }}>Next {horizon} days</span>
          </div>
        ...
        <div className="card" style={{ marginBottom: 0 }}>
          <div className="card-header">
            <div className="card-title">Cost forecast</div>
            <span style={{ fontSize: 11, color: '#aab8c0' }}>Next {horizon} days</span>
          </div>
          <div style={{ height: 200, position: 'relative' }}>
            <Bar
              ...
              return (
                <tr key={s.id}>
                  <td style={{ padding: 10, borderBottom: '0.5px solid #f0f4f0' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#1a2530' }}>{s.name}</div>
                    <div style={{ fontSize: 11, color: '#aab8c0' }}>{s.material}</div>
                  </td>
                  <td style={{ padding: 10, borderBottom: '0.5px solid #f0f4f0' }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: levelColor(s.level_pct) }}>{s.level_pct.toFixed(0)}%</span>
                  </td>
                  {[p7, p15, p30].map((p, i) => (
                  ...
                  <td style={{ padding: 10, borderBottom: '0.5px solid #f0f4f0' }}>
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 10, background: days <= 7 ? '#FCEBEB' : days <= 14 ? '#FAEEDA' : '#eaf5ee', color: days <= 7 ? '#A32D2D' : days <= 14 ? '#633806' : '#27500A' }}>
                      {days} days
                    </span>
                  </td>
                  <td style={{ padding: 10, borderBottom: '0.5px solid #f0f4f0' }}>
                    <span style={{ fontSize: 11, padding: '3px 9px', borderRadius: 10, fontWeight: 600, background: urgent ? '#FCEBEB' : days <= 15 ? '#FAEEDA' : '#eaf5ee', color: urgent ? '#A32D2D' : days <= 15 ? '#633806' : '#27500A' }}>
      ...
      <div className="card">
        ...
        <div className="grid-3" style={{ marginBottom: 0 }}>
          {scenarios.map((sc, i) => {
            ...
            const costPerAnimal = totalAnimals > 0
              ? (totalCostDay * sc.mult / totalAnimals).toFixed(2)
              : '—'
            return (
