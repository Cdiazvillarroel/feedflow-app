...
export default function AlertsPage() {
  ...
  const emptyContact = { name: '', position: '', phone: '', email: '', telegram_id: '', farm_id: activeFarmId, active: true }
  ...
  async function saveContact() {
    ...
    setSaving(true)
    const payload = { farm_id: contactForm.farm_id || activeFarmId, name: contactForm.name.trim(), position: contactForm.position || null, phone: contactForm.phone || null, email: contactForm.email || null, telegram_id: contactForm.telegram_id || null, active: contactForm.active }
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, width: 460, height: '100vh', background: '#fff', zIndex: 201, display: 'flex', flexDirection: 'column', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div>
                ...
                <input value={ruleForm.name} onChange={e => setRuleForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Critical level — Silo R1" style={iStyle(true)} />
              </div>
              ...
              <div style={{ background: '#f7f9f8', borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <input type="number" value={ruleForm.threshold} onChange={e => setRuleForm(p => ({ ...p, threshold: e.target.value }))} placeholder="e.g. 20" style={iStyle(true)} />
                  </div>
                </div>
                <div style={{ background: '#fff', borderRadius: 8, padding: '10px 12px', border: '0.5px solid #e8ede9' }}>
                  ...
                  <div style={{ fontSize: 12, color: '#1a2530', lineHeight: 1.6 }}>
                    Trigger when <strong>{metricLabel(ruleForm.metric)}</strong> {operatorLabel(ruleForm.operator)} <strong style={{ color: '#4CAF7D' }}>{ruleForm.threshold}{metricSuffix(ruleForm.metric)}</strong>
                    {ruleForm.silo_id ? <> on <strong>{siloName(ruleForm.silo_id)}</strong></> : <> on <strong>any silo</strong></>}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea value={ruleForm.notify_msg} onChange={e => setRuleForm(p => ({ ...p, notify_msg: e.target.value }))}
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, width: 420, height: '100vh', background: '#fff', zIndex: 201, display: 'flex', flexDirection: 'column', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div><label style={lStyle()}>Full name *</label><input style={iStyle(true)} value={contactForm.name} onChange={e => setContactForm(p => ({ ...p, name: e.target.value }))} placeholder="Juan Rodríguez" /></div>
              ...
              <div>
                ...
                <p style={{ fontSize: 11, color: '#aab8c0', marginTop: 4, lineHeight: 1.5 }}>
                  To get the chat ID, send a message to <strong>@feedflow_alerts_bot</strong> and use /start
                </p>
              </div>
