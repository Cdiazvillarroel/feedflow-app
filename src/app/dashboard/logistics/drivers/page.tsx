...
interface DeliveryRoute { id: string; driver_id: string | null; name: string; planned_date: string; status: string }
...
export default function DriversPage() {
  ...
  async function loadAll() {
    ...
    const [fmR, driversR, trucksR, routesR] = await Promise.all([
      ...
      supabase.from('delivery_routes').select('id, driver_id, name, planned_date, status').order('planned_date', { ascending: false }),
    ])
  ...
  const truckOf  = (id: string | null) => id ? trucks.find(t => t.id === id) : null
  const driverOf = (id: string | null) => id ? drivers.find(d => d.id === id) : null
    ...
    <>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, width: 440, height: '100vh', background: '#fff', zIndex: 201, display: 'flex', flexDirection: 'column', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <select value={driverForm.truck_id} onChange={e => setDriverForm(p => ({ ...p, truck_id: e.target.value }))} style={{ ...iStyle(true), background: '#fff' }}>
                  ...
                  {trucks.filter(t => t.active && (!driverForm.feed_mill_id || t.feed_mill_id === driverForm.feed_mill_id)).map(t => (
                    <option key={t.id} value={t.id}>{t.name} — {t.plate || 'No plate'} ({(t.capacity_kg/1000).toFixed(0)}t)</option>
                  ))}
                </select>
              </div>
              ...
              {isEditDriver && (
                <>
                  ...
                  {routes.filter(r => r.driver_id === (driverDrawer as Driver).id).slice(0, 5).map(r => {
                    ...
                    const s = sc[r.status] || sc.draft
                    return (
                      <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '0.5px solid #f0f4f0' }}>
                        <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 6, background: s.bg, color: s.color, fontWeight: 600, flexShrink: 0 }}>{r.status}</span>
                        <span style={{ fontSize: 12, color: '#1a2530', flex: 1 }}>{r.name}</span>
                        <span style={{ fontSize: 11, color: '#aab8c0' }}>{new Date(r.planned_date + 'T12:00:00').toLocaleDateString('en-AU', { day: 'numeric', month: 'short' })}</span>
                      </div>
                    )
                  })}
            ...
            <div style={{ padding: '16px 24px', borderTop: '0.5px solid #e8ede9', display: 'flex', gap: 10 }}>
              <button onClick={saveDriver} disabled={saving || !driverForm.name.trim()}
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, width: 420, height: '100vh', background: '#fff', zIndex: 201, display: 'flex', flexDirection: 'column', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={lStyle()}>Payload capacity (kg)</label>
                <input type="number" style={iStyle(true)} value={truckForm.capacity_kg} onChange={e => setTruckForm(p => ({ ...p, capacity_kg: e.target.value }))} placeholder="20000" step="1000" />
                <p style={{ fontSize: 11, color: '#aab8c0', marginTop: 4 }}>Used to calculate load for route planning</p>
              </div>
              ...
              {isEditTruck && (() => {
                ...
                return assignedDriver ? (
                  <>
                    <div style={{ height: '0.5px', background: '#e8ede9' }} />
                    <SecTitle title="Assigned driver" />
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '10px 12px', borderRadius: 8, border: '0.5px solid ' + (truckForm.active ? '#4A90C4' : '#e8ede9'), background: truckForm.active ? '#E6F1FB' : '#fff' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: '#aab8c0' }}>Available for route assignments</div>
                </div>
              </label>
            </div>

            <div style={{ padding: '16px 24px', borderTop: '0.5px solid #e8ede9', display: 'flex', gap: 10 }}>
              <button onClick={saveTruck} disabled={saving || !truckForm.name.trim()}
      ...
      <div className="page-header">
        ...
        <div className="page-actions">
          ...
          {view === 'fleet'   && <button className="btn-primary" style={{ background: '#4A90C4', borderColor: '#4A90C4' }} onClick={openNewTruck}>+ New truck</button>}
        </div>
      </div>
      ...
      <div className="summary-row">
        <div className="sum-card">
          ...
          <div className="sum-val green">{drivers.filter(d => d.active).length}</div>
          <div className="sum-sub">{drivers.filter(d => !d.active).length} inactive</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Assigned trucks</div>
          <div className="sum-val" style={{ color: '#4A90C4' }}>{drivers.filter(d => d.truck_id).length}</div>
          <div className="sum-sub">{drivers.filter(d => !d.truck_id && d.active).length} without truck</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Active trucks</div>
          <div className="sum-val" style={{ color: '#4A90C4' }}>{trucks.filter(t => t.active).length}</div>
          <div className="sum-sub">{trucks.reduce((sum, t) => sum + t.capacity_kg, 0) / 1000}t total capacity</div>
        </div>
        <div className="sum-card">
          <div className="sum-label">Routes this month</div>
          <div className="sum-val" style={{ color: '#633806' }}>{routes.filter(r => new Date(r.planned_date).getMonth() === new Date().getMonth()).length}</div>
          <div className="sum-sub">{routes.filter(r => r.status === 'in_progress').length} in progress</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '0.5px solid #e8ede9' }}>
        {[
          ...
          { key: 'fleet',   label: 'Fleet (' + trucks.length + ')'   },
        ].map(t => (
          <button key={t.key} onClick={() => setView(t.key as any)}
            style={{ padding: '9px 18px', fontSize: 13, fontWeight: view === t.key ? 600 : 400, cursor: 'pointer', border: 'none', background: 'transparent', fontFamily: 'inherit', color: view === t.key ? '#1a2530' : '#8a9aaa', borderBottom: view === t.key ? '2px solid #4CAF7D' : '2px solid transparent', marginBottom: -1 }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {view === 'drivers' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {drivers.length === 0 ? (
            ...
            const trk          = truckOf(d.truck_id)
            const driverRoutes = routes.filter(r => r.driver_id === d.id)
            const activeRoute  = driverRoutes.find(r => r.status === 'in_progress')
            const nextRoute    = driverRoutes.find(r => r.status === 'confirmed' && new Date(r.planned_date) >= new Date())
            return (
              <div key={d.id} onClick={() => openEditDriver(d)}
                style={{ display: 'flex', gap: 16, padding: '16px 20px', background: '#fff', borderRadius: 10, border: '0.5px solid #e8ede9', borderLeft: '3px solid ' + (d.active ? '#4CAF7D' : '#e8ede9'), cursor: 'pointer' }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.boxShadow = '0 2px 12px rgba(0,0,0,0.06)'}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.boxShadow = 'none'}>

                {/* Avatar */}
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: d.active ? '#1a2530' : '#e8ede9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, color: d.active ? '#fff' : '#aab8c0', flexShrink: 0 }}>
                  {d.name.charAt(0)}
                </div>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    ...
                    {activeRoute && <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 8, background: '#eaf5ee', color: '#27500A', fontWeight: 600 }}>🚛 On route</span>}
                  </div>
                  <div style={{ fontSize: 12, color: '#8a9aaa', marginBottom: 8 }}>
                    {d.license || 'No license'} · {millName(d.feed_mill_id)}
                    {d.phone ? ' · ' + d.phone : ''}
                  </div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {trk ? (
                      <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, background: '#E6F1FB', color: '#0C447C', fontWeight: 600 }}>
                        🚛 {trk.plate || trk.name} · {(trk.capacity_kg/1000).toFixed(0)}t
                      </span>
                    ) : (
                      <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, background: '#f0f4f0', color: '#aab8c0' }}>No truck assigned</span>
                    )}
                    {nextRoute && (
                      <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, background: '#FAEEDA', color: '#633806' }}>
                        Next: {new Date(nextRoute.planned_date + 'T12:00:00').toLocaleDateString('en-AU', { day: 'numeric', month: 'short' })}
                      </span>
                    )}
                    <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, background: '#f7f9f8', color: '#8a9aaa' }}>
                      {driverRoutes.length} route{driverRoutes.length !== 1 ? 's' : ''} total
                    </span>
                  </div>
                </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {trucks.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#8a9aaa' }}>
              ...
              <button onClick={openNewTruck} className="btn-primary" style={{ background: '#4A90C4' }}>+ Add first truck</button>
            </div>
          ) : trucks.map(t => {
            const assignedDriver = driverOf(drivers.find(d => d.truck_id === t.id)?.id || null)
            const truckRoutes    = routes.filter(r => r.driver_id && drivers.find(d => d.id === r.driver_id)?.truck_id === t.id)
            const activeRoute    = truckRoutes.find(r => r.status === 'in_progress')
            return (
              <div key={t.id} onClick={() => openEditTruck(t)}
                style={{ display: 'flex', gap: 16, padding: '16px 20px', background: '#fff', borderRadius: 10, border: '0.5px solid #e8ede9', borderLeft: '3px solid ' + (t.active ? '#4A90C4' : '#e8ede9'), cursor: 'pointer' }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.boxShadow = '0 2px 12px rgba(0,0,0,0.06)'}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.boxShadow = 'none'}>

                {/* Icon */}
                <div style={{ width: 48, height: 48, borderRadius: 10, background: t.active ? '#E6F1FB' : '#f0f4f0', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    ...
                    {activeRoute && <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 8, background: '#eaf5ee', color: '#27500A', fontWeight: 600 }}>In transit</span>}
                  </div>
                  <div style={{ fontSize: 12, color: '#8a9aaa', marginBottom: 8 }}>
                    {t.plate || 'No plate'} · {(t.capacity_kg/1000).toFixed(0)}t capacity · {millName(t.feed_mill_id)}
                  </div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {assignedDriver ? (
                      <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, background: '#eaf5ee', color: '#27500A', fontWeight: 600 }}>
                        👤 {assignedDriver.name}
                      </span>
                    ) : (
                      <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, background: '#f0f4f0', color: '#aab8c0' }}>No driver assigned</span>
                    )}
                    <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, background: '#f7f9f8', color: '#8a9aaa' }}>
                      {truckRoutes.length} route{truckRoutes.length !== 1 ? 's' : ''} total
                    </span>
                  </div>
                </div>

                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontSize: 20, fontWeight: 700, color: '#4A90C4' }}>{(t.capacity_kg/1000).toFixed(0)}t</div>
