...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: farmsError } = await supabaseAdmin.from('user_farms').insert(
      farm_ids.map((farm_id: string) => ({ user_id: userData.user.id, farm_id, role: role || 'viewer' }))
    )
    if (farmsError) return NextResponse.json({ error: farmsError.message }, { status: 400 })
