...
// Default farm ID — Granja Engorde
// In production this comes from the logged-in user's session
export const DEFAULT_FARM_ID = 'a1b2c3d4-0000-0000-0000-000000000001'
...
export async function getSilosWithReadings(farmId = DEFAULT_FARM_ID): Promise<SiloWithReading[]> {
  // Get latest readings from the view
  ...
  return silos.map(silo => {
    const lr = (latestReadings || []).find((r: SiloLatestReading) => r.silo_id === silo.id)
    const sensor = (sensors || []).find((s: Sensor) => s.silo_id === silo.id)

    const level_pct = lr?.level_pct ?? 0
    const kg_remaining = lr?.kg_remaining ?? 0
    const kgDay = 400 // will be replaced by real consumption calc
...
// Latest reading for a single silo
...
// Reading history for chart (last N days)
...
// Get daily consumption for a silo (kg/day average over last 7 days)
...
// All farm readings for analytics (last N days)
...
export async function getSensors(farmId = DEFAULT_FARM_ID): Promise<(Sensor & { silo_name: string; silo_lat: number | null; silo_lng: number | null })[]> {
  const { data, error } = await supabase
    .from('sensors')
    .select(`
      *,
...
export async function createAlert(alert: Omit<Alert, 'id' | 'acknowledged' | 'triggered_at' | 'acked_at' | 'acked_by'>): Promise<boolean> {
  const { error } = await supabase
    .from('alerts')
