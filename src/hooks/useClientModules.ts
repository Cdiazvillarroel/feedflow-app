...
export function useClientModules(userId: string | null) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!cuData) {
        ...
        // Usuario sin cliente — solo dashboard básico
        const fallback = new Map<ModuleKey, ClientModule>()
        fallback.set('dashboard', {
          module:        'dashboard',
          is_enabled:    true,
          feature_level: 'basic',
          source:        'PLAN',
  ...
  const canEdit      = (key: ModuleKey) => {
    const l = featureLevel(key)
    return l === 'full' || l === 'ai'
  }
